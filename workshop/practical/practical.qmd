...
The full model becomes the following:

$$
\hat{\theta_i} = \beta_0 + u_i + \epsilon_i
$$

In this case, the intercept-only model assumes that the estimated income ($\hat{\theta_i}$) is a combination of the average income across all districts ($\beta_0}) and the 

This model does not include any auxiliary variables, and assumes we do not have any specific data to predict the districts' estimates. The intercept-only model assumes that the best guess for our estimator is the overall value of the variable (e.g. the average country income). However, we know that each area will have its own unique area-level variance, which will deviate the value of their esitmate from the global average. 
